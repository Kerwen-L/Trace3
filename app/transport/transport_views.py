...
def Transport_start(request):
    if request.method=="POST":
        ...
        PackedUcl = dict_get['ucl']                                                        # ucl包
        serialnumber = dict_get['serialnumber']                                            # 顺序号
        productionId = dict_get['productionId']                                            # 生产内容id
        flag = dict_get['flag']                                                            # 环节标志为 transport
    ...
    if temp_person:
        ...
        if flag:  # 先填写基础数据
            models.TransportData.objects.filter(TransactionPersonID=person_id,State=0,ProductionID=productionId).update(      # 更新前端推送内容到运输数据表
                ...
                State=1, # 标志商品状态，0表示已经扫码录入，1表示已经开始运输，2表示运输到达
                Flag=2,  # 田杨静需要使用的
            )
            if dict_get['From'].find("牧场") >= 0:                                                 #模糊查询 说明运输员在生产运输阶段
                print("开始更新生产数据 环节标志30")
                list1 = models.TransportData.objects.filter(TransactionPersonID=person_id,State=1,ProductionID=productionId)  #取出本次运输的全部生产商品记录
                for record in list1:
                ...
                list1 = models.TransportData.objects.filter(TransactionPersonID=person_id,State=1,ProductionID=productionId)  #取出本次运输的全部检疫商品记录
                for record in list1:
                    models.TransportData.objects.filter(ProductionID=record.ProductionID).update(  #填写流通编号
                ...
                list1 = models.TransportData.objects.filter(TransactionPersonID=person_id,State=1,ProductionID=productionId)  #取出本次运输的全部加工商品记录
                for record in list1:
                    models.TransportData.objects.filter(ProductionID=record.ProductionID).update(  #填写流通编号
            ...
            return HttpResponse("该运输员没有运输数据，或者您填写的生产内容id在数据库中不存在，请找后端确认")
    else:
